...
Ketiga program menggunakan NumPy dan Matplotlib. Jalankan program dari direktori `Kode` agar gambar disimpan ke jalur relatif `../Gambar` yang digunakan dalam dokumen ini.

```bash
python -m pip install numpy matplotlib
python 01_klasifikasi_sinyal.py
python 02_dekomposisi_genap_ganjil.py
python 03_transformasi_waktu.py
```

Berkas kode mandiri tersedia di direktori `Kode`, sedangkan gambar PNG tersedia di direktori `Gambar`. Paket lengkap mempertahankan struktur direktori tersebut agar seluruh tautan gambar pada berkas Markdown dapat ditampilkan dengan benar.
